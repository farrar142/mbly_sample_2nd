...
def deploy():
    ...
    #     shut_img_con(test_con.container_name,cur_image_name)
        ...
        print("7.배포컨테이너 생성")
        os.system(f"docker run -d -p {deploy_port}:{deploy_port} --name {deploy_con_name} {cur_image_name} -v $PWD/media /media -v $PWD/static /static  --restart unless-stopped gunicorn --bind 0:{deploy_port} {path}.wsgi")
